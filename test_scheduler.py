...
def display_schedule(sol_assignment, schedulercsp):
    ...
    for course_name in schedulercsp.course_info_dict:
        pref_prof_list, student_count, duration, before_courses_list = schedulercsp.course_info_dict[course_name]
        print(f'{course_name}: {pref_prof_list}, {student_count}, {duration}, {before_courses_list}')
...
if __name__ == "__main__":    
    ...
    with open('./scheduler_data_0.pickle', 'rb') as handle:
        courses, loc_info_dict, course_info_dict, time_slots = pickle.load(handle) # read in courses, loc_info_dict, course_info_dict, time_slots
    
    cspscheduler = SchedulerCSP(courses, loc_info_dict, course_info_dict, time_slots)
    ...
    for i in range(2):
        ...
        with open(fname, 'rb') as handle:
            courses, loc_info_dict, course_info_dict, time_slots = pickle.load(handle) # read in the schedulercsp object
